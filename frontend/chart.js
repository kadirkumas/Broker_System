...
setInterval(() => {
    if (Date.now() - wsWatchlistLastUpdate > 20000) window.updateWatchlistsRest();
}, 1000);
...
window.signalFilter = localStorage.getItem('cryptoSignalFilter') || null; // 'signal' | 'close' | null
window.signalPage = 1;
        ...
        if (window._signalInitialized) {
            events.forEach(evt => {
                ...
                if (!window._seenSignalIds.has(uid)) {
                    if (evt.event_type === 'signal') {
                        const isLong = evt.signal === 'LONG';
                        window.showToast(`${evt.symbol} [${evt.strategy}] ${evt.signal}`, isLong ? 'success' : 'error', 5000);
                    } else {
                        const sign = evt.pnl_amount >= 0 ? '+' : '';
                        window.showToast(`${evt.symbol} KAPANDI ${sign}${evt.pnl_amount.toFixed(4)} USDT`, evt.pnl_amount >= 0 ? 'success' : 'error', 5000);
                    }
                }
            });
        }
